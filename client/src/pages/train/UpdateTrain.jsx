...
export default function UpdateTrain() {
    ...
    useEffect(() =>{
        const fetchTrain = async () => {
            ...
            const res = await fetch(`/api/train/get/${trainID}`);
            const data = await res.json();
            if(data.success === false) {
                console.log(data.message);
                return;
            }
            setFormData(data.traindetails);
            console.log(trainID);
        }
        fetchTrain();
    }, []);
    ...
    <form className='flex flex-col sm:flex-row gap-4' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-4 flex-1'>
            ...
            <div className='flex flex-wrap gap-6'>
                <div className='flex items-center gap-2'>
                    <input type='text' id='from' min='1' max='20' className='p-3 border border-blue-300 rounded-lg' required onChange={handleChange} value={formData.from}/>
                    <p>Start Station</p>
                </div>
                <div className='flex items-center gap-2'>
                ...
                <div className='flex items-center gap-2'>
                    <input type='text' id='destination' min='1' max='20' className='p-3 border border-blue-300 rounded-lg' required onChange={handleChange} value={formData.destination} />
                    <p>Destination</p>
                </div>
                <div className='flex items-center gap-2'>
            ...
            <button className='p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80' disabled={loading}>{loading ? 'Loading' : 'Update the Train'}</button>
            <button className='p-3 bg-red-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80'>Cancel</button>

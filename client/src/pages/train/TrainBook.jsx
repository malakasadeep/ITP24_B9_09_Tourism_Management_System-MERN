...
export default function TrainBook() {
    ...
    useEffect(() => {
        const getOneTrain = () => {
            axios.get(`/api/train/get/${id}`).then((res) => {
  ...
  return (
    <div className="min-h-screen mt-6">
            <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:py-24 lg:px-8">
                <div className="flex flex-col md:flex-row bg-white shadow-xl rounded-xl overflow-hidden">
                    ...
                    <div className="md:w-1/2 p-8 flex flex-col justify-center items-center">
                        ...
                        <Link>
                            <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md w-full md:w-[350px] lg:w-[300px]" onClick={() => openModal(singleTrain._id)}>
                                BOOK
                            </button>
                        </Link>
                    </div>
            ...
            <Modal isOpen={modalIsOpen} onRequestClose={() => setModalIsOpen(false)} className="mt-28">
    <div className="modal-content bg-skyblue-200 rounded-lg p-6 relative mt-4 ml-4 mr-4 overflow-y-auto" style={{ maxHeight: "80vh" }}>
        <button className="close-btn  top-30 right-10 mt-4 mr-4 bg-skyblue-500 text-white rounded-full w-8 h-8 flex items-center justify-center hover:bg-skyblue-600 fixed" onClick={() => setModalIsOpen(false)}>X</button>
        ...
        <div className="seats-container grid grid-cols-4 gap-4">
            {seats.map((seat, index) => (
                <div key={index} className={`seat rounded-lg p-3 flex items-center justify-center ${selectedSeats.some(selectedSeat => selectedSeat.seatNumber === seat.seatNumber) ? 'bg-blue-500 cursor-not-allowed' : (seat.available ? 'bg-red-200 cursor-pointer' : 'bg-green-200 cursor-pointer')}`} onClick={() => !seat.available ? handleSeatClick(seat) : null}>
        ...
        <div className="flex justify-between items-center mt-4">
            ...
            <p className="text-lg font-semibold">Total Cost: ${selectedSeats.length * singleTrain.price}</p>
            <div>
                <button className="book-btn bg-sky-500 text-white rounded-lg px-4 py-2 hover:bg-skyblue-600" onClick={handleBookSeat}>Book the Seats</button>
            </div>
        </div>
    </div>
</Modal>

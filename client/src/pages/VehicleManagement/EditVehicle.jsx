...
const AddVehicle = () => {
  ...
  useEffect(() => {
    const fetchvehicle = async () => {
      const vId = params.id;
      const res = await fetch(`/api/vehicle/get-vehi/${vId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const regex =
      /^(?:[A-Z]{3}-\d{4}|^\d{2}-\d{4}|^[A-Z]{2}-\d{4}|^\d{1}-\d{4}|^\d{2}\SRI-\d{4})$/;
      ...
      setLoading(true);
      setError(false);
      const res = await fetch(`/api/vehicle/update/${params.id}`, {
        method: "PUT",
        headers: {
      ...
      if (data.success === false) {
        ...
        if (currentUser.isadmin) {
          ...
          navigate(`/Vehicle/get/${formData._id}`);
        }
      }
    } catch (error) {
    ...
    <main className="p-3 max-w-4xl mx-auto">
      ...
      <form className="flex flex-col sm:flex-row gap-4" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-4 flex-1">
          ...
          <select
            className="p-2 border rounded-md w-full bg-white mt-2"
            id="type"
            onChange={handleChange}
            value={formData.type}
            required
          ...
          <label for className="lg:text-lg text-left mt-6">
            Number of Seats
          </label>
          <input
            type="number"
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? "Loading" : "Update the Vehicle"}
          </button>

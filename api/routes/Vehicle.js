...
router.route("/update/:id").put(async (req, res) => {
  ...
  const update = await Vehicle.findByIdAndUpdate(vehiId, updateVehicle)
    .then(() => {
...
router.route("/delete/:id").delete(async (req, res) => {
  ...
  await Vehicle.findByIdAndDelete(vehiId)
    .then(() => {
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "Error with delete vehicle", erroe: err.message });
    });
});
